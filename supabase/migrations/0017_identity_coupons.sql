...
create table public.coupons (id uuid primary key default gen_random_uuid(), store_id uuid not null references public.stores(id) on delete cascade, code text not null, type public.coupon_type not null, value numeric(18,4) not null check (value > 0), min_subtotal numeric(18,2) not null default 0 check (min_subtotal >= 0), max_discount numeric(18,2), starts_at timestamptz, ends_at timestamptz, usage_limit integer, usage_count integer not null default 0, per_customer_limit integer not null default 1, active boolean not null default false, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique(store_id, lower(code)), check (ends_at is null or starts_at is null or ends_at > starts_at), check ((type='percentage' and value <= 100) or type='fixed'));
