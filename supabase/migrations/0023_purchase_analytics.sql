...
begin
  ...
  if nullif(trim(coalesce(p_customer->>'auth_user_id', '')), '') is not null then
    v_auth_user_id := nullif(trim(p_customer->>'auth_user_id'), '')::uuid;
  end if;
  ...
  insert into public.customers (store_id, email, first_name, last_name, phone, auth_user_id)
  values (v_store.id, v_email, v_first_name, v_last_name, v_phone, v_auth_user_id)
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'source', case when v_auth_user_id is null then 'guest_checkout' else 'signed_checkout' end,
      'item_count', jsonb_array_length(v_resolved_items)
    )
  );

  insert into public.analytics_events(
    store_id, session_id, event_name, path, order_id, value, currency, metadata
  )
  values(
    v_store.id,
    p_idempotency_key,
    ...
    '/checkout/success',
    v_order_id,
    v_subtotal + v_shipping_total + case when v_tax_inclusive then 0 else v_tax_total end,
    v_currency,
